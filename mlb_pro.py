...
def pobierz_statystyki_druzyn_mlb():
    ...
    print("📊 Pobieram statystyki (K-Rate i Team ERA)...")
    headers = {'User-Agent': 'Mozilla/5.0'}
    
    # 1. HITTING STATS (dla Pitcherów: jak często rywale łapią Strikeouty)
    url_hit = f"https://statsapi.mlb.com/api/v1/teams/stats?season={SEZON_MLB}&group=hitting&stats=season&gameType=R"
    try:
        ...
        if stats_h and stats_h[0].get('splits'):
            ...
            for team in stats_h[0]['splits']:
                ...
                CACHE_TEAM_K_RATE[team['team']['name']] = k / pa if pa > 0 else 0
                total_k += k; total_pa += pa
            if total_pa > 0: LEAGUE_AVG_K_RATE = total_k / total_pa
    except: pass

    # 2. PITCHING STATS (dla Pałkarzy: jak dobrze broni drużyna przeciwna - ERA)
    url_pitch = f"https://statsapi.mlb.com/api/v1/teams/stats?season={SEZON_MLB}&group=pitching&stats=season&gameType=R"
    try:
        ...
        if stats_p and stats_p[0].get('splits'):
            ...
            for team in stats_p[0]['splits']:
                era = float(team['stat'].get('era', 4.20))
                CACHE_TEAM_ERA[team['team']['name']] = era
                total_era += era; count += 1
            if count > 0: LEAGUE_AVG_ERA = total_era / count
            print(f"✅ Średnie ligowe: K-Rate={round(LEAGUE_AVG_K_RATE*100,1)}% | ERA={round(LEAGUE_AVG_ERA,2)}")
    except: pass
    ...
    for s in [SEZON_MLB, SEZON_MLB - 1]:
        try:
            ...
            if 'stats' in res and len(res['stats']) > 0:
                for game in res['stats'][0].get('splits', []):
                    st = game.get('stat', {})
...
def uruchom_mlb_pro():
    print("==================================================")
    print("🚀 QUANT AI BOTS: MLB PRO ULTIMATE v4.1 (Team Defense+)")
    print("==================================================")
    ...
    try:
        ...
        if isinstance(events, dict) and 'message' in events: return []
    except: return []
        ...
        for bm in res_odds.get('bookmakers', []):
            for mkt in bm.get('markets', []):
                ...
                for oc in mkt['outcomes']:
                    ...
                    player_id = None; is_today_home = False; opp_name = ""; bat_side = 'R'; b_order = 0
                    ...
                    if rola == 'pitcher':
                        if p_clean in h_clean or h_clean in p_clean: player_id = dane_oficjalne['home_pitcher_id']; is_today_home = True; opp_name = ev['away_team']
                        elif p_clean in a_clean or a_clean in p_clean: player_id = dane_oficjalne['away_pitcher_id']; is_today_home = False; opp_name = ev['home_team']
                    else:
                        h_roster = {p['person']['fullName'].lower().replace(".", "").strip(): p['person'] for p in requests.get(f"https://statsapi.mlb.com/api/v1/teams/{dane_oficjalne['home_team_id']}/roster").json().get('roster', [])}
                        a_roster = {p['person']['fullName'].lower().replace(".", "").strip(): p['person'] for p in requests.get(f"https://statsapi.mlb.com/api/v1/teams/{dane_oficjalne['away_team_id']}/roster").json().get('roster', [])}
                        
                        if p_clean in h_roster:
                            player_id = h_roster[p_clean]['id']; bat_side = h_roster[p_clean].get('batSide', {}).get('code', 'R')
                            is_today_home = True; opp_name = ev['away_team']
                            b_order = dane_oficjalne.get('lineups_home', {}).get(player_id, 0)
                        elif p_clean in a_roster:
                            player_id = a_roster[p_clean]['id']; bat_side = a_roster[p_clean].get('batSide', {}).get('code', 'R')
                            is_today_home = False; opp_name = ev['home_team']
                    ...
                    if rola == 'pitcher':
                        opp_k_rate = CACHE_TEAM_K_RATE.get(opp_name, LEAGUE_AVG_K_RATE)
                        korekta *= max(0.85, min(1.15, opp_k_rate / LEAGUE_AVG_K_RATE))
                        if datetime.now().month == 4: korekta *= 0.92; uwagi += " 📉 Limit rzutów (Kwiecień)."
                        m_color = "rank-green" if korekta > 1.05 else "rank-red"
                        m_rank = f"K-Rate rywala: {round(opp_k_rate*100,1)}%"
                    else:
                        # Zmiana: Dołączamy ERA całego zespołu (Miotacz Startowy + Bullpen)
                        opp_era = CACHE_TEAM_ERA.get(opp_name, LEAGUE_AVG_ERA)
                        era_korekta = max(0.85, min(1.15, opp_era / LEAGUE_AVG_ERA))
                        korekta *= era_korekta
                        uwagi += f" ⚾ Rywal ERA: {round(opp_era, 2)}."
                        
                        pf = PARK_FACTORS.get(ev['home_team'], 1.0)
                        if mlb_stat_key == 'homeRuns': pf = ((pf - 1.0) * 1.5) + 1.0 
                        korekta *= pf
                        
                        p_hand = dane_oficjalne['home_pitcher_hand'] if not is_today_home else dane_oficjalne['away_pitcher_hand']
                        if p_hand and bat_side:
                            if bat_side == 'S': korekta *= 1.04
                            elif bat_side != p_hand: korekta *= 1.08; m_color = "rank-green"; m_rank = "Dobry Split"
                            else: korekta *= 0.95; m_color = "rank-red"; m_rank = "Zły Split"
                    ...
                    # 3. FILTR MINIMALNEJ JAKOŚCI (55% Szans)
                    ...
                    wyniki.append({
                        ...
                        "l5": f"{pokrycie_l5}%", "l10": f"{int((sum(1 for x in vals[-10:] if x > linia)/10)*100)}%",
    ...
    if top:
        msg = "🚨 <b>RAPORT QUANT AI: MLB (PRO ULTIMATE v4.1)</b> 🚨\n\n"
        for t in top: 
    ...
    wyslij_plik_na_githuba(MLB_JSON_FILE, "MLB Data: Update v4.1 (WMA + Splits + ERA)")
    return wyniki
